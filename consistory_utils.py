...
class AttentionStore:
    def __init__(self):
        self.attention_maps = {'target': None, 'reference': None}
    
    def __call__(self, attn_map, is_target=True):
        key = 'target' if is_target else 'reference'
        self.attention_maps[key] = attn_map.detach()
    
    def get_attention_maps(self):
        return self.attention_maps

class AttentionMasker(nn.Module):
    def __init__(self, mask_scale=2.0):
        super().__init__()
        self.mask_scale = mask_scale
    
    def forward(self, hidden_states, x=None):
        return hidden_states * self.mask_scale
