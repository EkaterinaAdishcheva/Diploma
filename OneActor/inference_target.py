...
def main():
    ...
    _str = config['target_prompt'] + " " + config['add_target_prompt']
    ...
    token_id = find_token_ids(pipeline.tokenizer, config['target_prompt'], config['base'])
    generator = torch.manual_seed(config['seed'])
    ...
    if config['only_step'] is False:
        for i in range(50):
            ...
            image = pipeline_inference(
                pipeline, 
                config['target_prompt'] + " " + config['add_target_prompt'],
                config['target_neg_prompt'],
                config, oneactor_extra_config)
            image = image.images[0]
            image.save(f"{out_root}/inference/target_step_{steps}.jpg")
        ...
        image = pipeline_inference(
            ...
            config, oneactor_extra_config)
        image = image.images[0]
        image.save(f"{out_root}/inference/target_step_best.jpg")
    else:
        ...
        for steps in steps_list:
            ...
            image = pipeline_inference(
                ...
                config, oneactor_extra_config)
            image = image.images[0]
            image.save(f"{out_root}/inference/target_step_{str(steps)}.jpg")
