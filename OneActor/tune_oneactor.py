...
class OneActorDataset(Dataset):
    def __init__(
        ...
        self.learnable_property = 'character'
        self.size = config['size']
        self.base_condition = config['base']
        self.flip_p = flip_p
        self.neg_num = config['neg_num']

        self.image_paths = [f"{self.data_root}/target.jpg"]
        ...
        with open(self.data_root+'/target_data.pkl', 'rb') as f:
...
def main():
    ...
    if accelerator.is_main_process:
        if output_dir is not None:
            ...
            os.makedirs(f"{output_dir}/{args.model_path}/ckpt", exist_ok=True)
            os.makedirs(f"{output_dir}/{args.model_path}/weight", exist_ok=True)
        shutil.copyfile(args.config_path, f"{output_dir}/{args.model_path}/train_config.yaml")
        shutil.copyfile(args.prompt_path, f"{output_dir}/{args.model_path}/train_prompt.yaml")
        with open(f"{output_dir}/{args.model_path}/log_train.yaml", 'w') as outfile:
    ...
    vae.to(accelerator.device, dtype=torch.float32) # fo not work with fp16
    ...
    if config['resume_from_checkpoint'] is not None:
        ...
        dirs = os.listdir(f"{output_dir}/{args.model_path}/ckpt")
        ...
        if path is None:
            ...
            accelerator.load_state(os.path.join(f"{output_dir}/{args.model_path}", 'ckpt', path))
    ...
    for epoch in range(first_epoch, config['epochs']):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(projector):
                ...
                latents = vae.encode(batch["pixel_values"][0].to(dtype=torch.float32)).latent_dist.sample().detach()
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % config['save_steps'] == 0:
                    ...
                    save_path = os.path.join(f"{output_dir}/{args.model_path}", 'weight', weight_name)
                ...
                if accelerator.is_main_process:
                    if global_step % config['checkpointing_steps'] == 0:
                        save_path = os.path.join(f"{output_dir}/{args.model_path}", 'ckpt', f"checkpoint-{initial_global_step + global_step}")
